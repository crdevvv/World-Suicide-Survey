...
```
* è evidente un'elevata correlazione fra il GDP totale e gli anni passati
...
* l'**eutanasia** non viene considerata come suicidio in questo dataset,in quanto è considerata principalmente una procedura medica.
...
* gli uomini hanno un tasso di suicidio circa quattro volte superiore alle donne,tuttavia è affermato che le donne tentano il suicidio leggermente di più rispetto agli uomini perché sono maggiormente diagnosticate come affette da depressione.


* è stato riscontrato che i fattori di rischio come disoccupazione, pensionamento ed essere single sono tutti fattori di rischio significativi per il suicidio degli uomini, mentre sono molto meno significativi nelle donne.
...
* questo risultato conferma l'ipotesi iniziale in quanto c'è una forte correlazione (0.880) fra il *gdp pro capite* e lo scorrere     degli anni,che presuppone lo sviluppo economico di ogni stato.
...
* il *coefficiente di determinazione* è leggermente più alto ma non è sufficiente per rendere affidabile il modello.
Tuttavia il *p-value* (0.05) è minore rispetto alla soglia tipica (0.05) di rifiuto dell'ipotesi nulla, e quindi possiamo rifiutare l'ipotesi nulla tale per cui il Gdp di una nazione non è in relazione con il tasso di suicidi
...
$suicidi(100k) = 8.337 + ( 1.066*10^-4 ) * GDP(procapite)$
...
## Suicidi in relazione a genere e GDP pro capite
...
* visualizziamo il modello senza *outliers*
...
```
* con un **R^2** pari a *0.51* questo modello è abbastanza affidabile e ha un buon potere predittivo per quanto riguarda la variabile risposta *suicidi_per_100k* rispetto alla variabile esplicativa *sex*.
...
* inoltre il *p-value* con un valore ben sotto la soglia minima di rifiuto dell'ipotesi nulla ci permette di confermare l'ipotesi fatta all'inizio dell'analisi.
